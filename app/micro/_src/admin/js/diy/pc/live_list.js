...
$(function(){
	...
	var object_type;
    //课程列表加载初始数据
    let live_list_loader = function(rows=5, order_field= 'create_time', order_type= 'ASC', category_id= 0, element){
	...
	//点击显示直播列表控制区
	$('.page-diy-pc-section').on("click",'[data-type="live_list"]',function(e){
		...
		$('.page-diy-pc-section').on('click','[data-object-index="'+object_index+'"] .btn',function(){
			...
			//console.log(rank);
			//执行重新加载ajax数据
			live_list_loader(rows,order_field,order_type,category_id,'[data-object-index='+object_index+']');
		});
     ...
     */
    $('.page-diy-pc-section .object-lists').on("click",'.btn-object[data-type="live_list"]',function(){
        ...
        $.get(url, function($res){
            ...
            if($res.code == 200){
                ...
                //获取初始列表数据
                live_list_loader(5,'create_time','ASC',0,'[data-object-index='+object_index+']');
            }else{
