...
$(function(){
	//默认加载接口数据
    var list_api = $('.btn-object[data-type="knowledge_list"]').data('list-api');
	//初始化组件索引
	var object_index;
	//初始化组件类型
    var object_type;
    //课程列表加载初始数据
    var knowledge_list_loader = function(rows=5, order_field= 'create_time', order_type= 'DESC', type= 'all', category_id= 0, element){
        ...
        let api = $('input[name="knowledge_api"]').val();
        //默认加载接口数据
        let url = list_api + '?alone_sale=1&type='+type+'&category_id='+category_id+'&rows='+rows+'&order_field='+order_field+'&order_type='+order_type;
        let html_text = '';
        $.get(url,function(data){
            ...
            if(data.code){
                ...
                $(element).find('.knowledge-list-preview .list').addClass('rank0');
                $(element).find('.knowledge-list-preview .list').html(html_text);
            }
        });
    }

	//点击显示列表控制区
	$('.page-diy-pc-section').on("click",'[data-type="knowledge_list"]',function(e){
		...
		//点击确认按钮后的数据重新加载
		$('.page-diy-pc-section').on('click','[data-object-index="'+object_index+'"] .btn-submit',function(){
			//点击确认按钮后的数据重新加载
			...
			//console.log(rank);
			//执行重新加载ajax数据
			knowledge_list_loader(rows,order_field,order_type,type,category_id,'[data-object-index='+object_index+']');
        });

        //点击关闭按钮后隐藏控制区
		$('.page-diy-pc-section').on('click','[data-object-index="'+object_index+'"] .btn-close',function(e){
            e.stopPropagation();
			//点击确认按钮后的数据重新加载
			$('[data-object-index="'+object_index+'"]').find('.diy-preview-controller').removeClass('show');
        });
     ...
     */
    $('.page-diy-pc-section .object-lists').on("click",'.btn-object[data-type="knowledge_list"]',function(){
        ...
        if(type=='knowledge_list'){
            knowledge_list_loader(5,'create_time','ASC','all',0,'[data-object-index='+object_index+']');
        }
    });
