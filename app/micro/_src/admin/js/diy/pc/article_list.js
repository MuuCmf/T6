...
$(function(){
	...
	var object_type;
    // 列表接口
    var list_api = $('.btn-object[data-type="article_list"]').data('list-api');
    
    //文章列表加载初始数据
    let article_list_loader = function(rows=2, category_id= 0, order_field= 'create_time', order_type= 'DESC', style = 0,element){

        //默认加载接口数据
        let url = list_api + '?rows='+rows+ '&category_id='+category_id+'&order_field='+order_field+'&order_type='+order_type;
        let html_text = '';
								...
								html_text += '<div class="content">';
									...
									html_text += '<div class="info">';
										...
										if(show_view == 1){
											...
											html_text += '    <i class="fa fa-eye" aria-hidden="true"></i> ' + n.handling_view;
											html_text += '</div>';
										}
										html_text += '</div>';
									html_text += '</div>';
	...
	//点击显示文章列表控制区
	$('.page-diy-pc-section').on("click",'[data-type="article_list"]',function(){
        console.log('显示文章列表控制区');
		//已经显示的不再触发
		...
		$('.page-diy-pc-section').on('click','[data-object-index="'+object_index+'"] .btn',function(){
            ...
            console.log(style);
			//执行重新加载ajax数据
			article_list_loader(rows, category_id, order_field, order_type, style,'[data-object-index='+object_index+']');
		});
		...
		//分类数据获取
		var category_url = $('[data-object-index="'+object_index+'"]').data('category-api');
		var category_html = '';
     ...
     */
    $('.page-diy-pc-section .object-lists').on("click",'.btn-object[data-type="article_list"]',function(){
        ...
        $.get(url, function($res){
            ...
            if($res.code == 200){
                ...
                //获取初始列表数据
                article_list_loader(5,0,'create_time','ASC',0,'[data-object-index='+object_index+']');
            }else{
                toast.error('该组件需安装文章营销插件PC端支持','danger');
                return;
            }
        });
