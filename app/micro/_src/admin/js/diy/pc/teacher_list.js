...
$(function(){
    ...
    let teacher_list_loader = function(rows=2, order_field= 'create_time', order_type= 'DESC', rank = 1, element){
        ...
        let api = $('input[name="teacher_api"]').val();
        //默认加载接口数据
        let url = api + '?rows='+rows+'&order_field='+order_field+'&order_type='+order_type;;
        let html_text = '';
	...
	//点击显示专栏列表控制区
	$('.page-diy-pc-section').on("click",'[data-type="teacher_list"]',function(){
		//已经显示的不再触发
     ...
     */
    $('.page-diy-pc-section .object-lists').on("click",'.btn-object[data-type="teacher_list"]',function(){
