$(function(){
    ...
    var object_type;
    //列表数据接口
    var list_api = $('.btn-object[data-type="scoreshop_goods_list"]').data('list-api');
    //列表加载初始数据
    let scoreshop_goods_list_loader = function(rows=5, order_field= 'create_time', order_type= 'DESC', type= 'all', category_id= 0, element){
        ...
        //let api = list_api;
        //默认加载接口数据
        let url = list_api + '&type='+type+'&category_id='+category_id+'&rows='+rows+'&order_field='+order_field+'&order_type='+order_type;
        let html_text = '';
        $.get(url,function(data){
            ...
            if(data.code){
                if(data.data){
                    $.each(data.data.data,function(i,n){
                        ...
                        if(rank == 1){
                            ...
                            html_text += '<div class="price"><span class="score">'+ n.price +'</span></div>';
                            ...
                            if(show_sale == 1){
                                ...
                                html_text += '   <i class="fa fa-shopping-bag" aria-hidden="true"></i> ' + n.sell_num;
                                html_text += ' </div>';
                            }
                             if(show_favorites == 1){
                                html_text += ' <div class="favorites">';
                                html_text += '   <i class="fa fa-star-o" aria-hidden="true"></i> ';
                                html_text += '   <span class="favorites-num">' + n.favorite + '</span>'; 
                                html_text += '</div>';
                            }
                            html_text += '</div>';
                            html_text += '</div>';
                ...
                if(rank == 1){
                    $(element).find('.scoreshop-goods-list-preview .list').removeClass('rank0');
                    $(element).find('.scoreshop-goods-list-preview .list').addClass('rank1');
                }else{
                    $(element).find('.scoreshop-goods-list-preview .list').removeClass('rank1');
                    $(element).find('.scoreshop-goods-list-preview .list').addClass('rank0');
                }

                $(element).find('.scoreshop-goods-list-preview .list').html(html_text);
            }
        });
    }

    //点击显示积分商品列表控制区
    $('.page-diy-pc-section').on("click",'[data-type="scoreshop_goods_list"]',function(e){
        ...
        $('.page-diy-pc-section').on('click','[data-object-index="'+object_index+'"] .btn',function(){
            ...
            //console.log(rank);
            //执行重新加载ajax数据
            scoreshop_goods_list_loader(rows,order_field,order_type,type,category_id,'[data-object-index='+object_index+']');
        });
        ...
        //课程分类数据获取
        //console.log(url);
        var category_url = $(this).data('category-url');
        var category_html = '';
     ...
     */
    $('.page-diy-pc-section .object-lists').on("click",'.btn-object[data-type="scoreshop_goods_list"]',function(){
        ...
        $.get(url, function($res){
            ...
            if($res.code == 200){
                ...
                scoreshop_goods_list_loader(5,'create_time','ASC','all',0,'[data-object-index='+object_index+']');
                //}
            }else{
                toast.error('该组件需安装积分商城模块','danger');
                return;
            }
        });
    });
