...
$(function(){
    ...
    var object_type;
    //列表数据接口
    var list_api = $('.btn-object[data-type="material_list"]').data('list-api');
    //课程列表加载初始数据
    let material_list_loader = function(rows=2, order_field= 'create_time', order_type= 'ASC', category_id= 0, rank = 1, style = 0, element){
        ...
        $.get(url,function(data){
            ...
            if(data.code){
                ...
                if(rank == 1){
                    $(element).find('.material-list-preview .list').removeClass('rank0');
                    $(element).find('.material-list-preview .list').addClass('rank1');
                }else{
                    $(element).find('.material-list-preview .list').removeClass('rank1');
                    $(element).find('.material-list-preview .list').addClass('rank0');
                }

                $(element).find('.material-list-preview .list').html(html_text);
            }
        });
    }

	//点击显示列表控制区
	$('.page-diy-pc-section').on("click",'[data-type="material_list"]',function(e){
		...
		$('.page-diy-pc-section').on('click','[data-object-index="'+object_index+'"] .btn',function(){
			...
			console.log(rank);
			//执行重新加载ajax数据
			material_list_loader(rows,order_field,order_type,category_id,rank,style,'[data-object-index='+object_index+']');
		});
        ...
        //排列布局选项选择触发事件
		$('.page-diy-pc-section').on('change','[data-object-index="'+object_index+'"] [name="rank"]',function(){
            //alert('排列布局选择');
            var rank = $(this).val();
            if(rank == 1){
                //alert('选择了竖排');
     ...
     */
    $('.page-diy-pc-section .object-lists').on("click",'.btn-object[data-type="material_list"]',function(){
        ...
        //获取初始列表数据
        material_list_loader(5,'create_time','ASC',0,1,0,'[data-object-index='+object_index+']');
    });
