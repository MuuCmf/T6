...
$(function(){
	...
	//var api = '/knowledge/api/columnlist';
    ...
    var object_type;
    //专栏列表加载初始数据
    var column_list_loader = function(rows=5, category_id= 0, order_field= 'create_time', order_type= 'DESC', element){
        ...
        let api = $('input[name="column_api"]').val();
        //默认加载接口数据
	...
	//点击显示专栏列表控制区
	$('.page-diy-pc-section').on("click",'[data-type="column_list"]',function(){
		//已经显示的不再触发
		...
		$('.page-diy-pc-section').on('click','[data-object-index="'+object_index+'"] .btn',function(){
			...
			//执行重新加载ajax数据
			column_list_loader(rows, category_id, order_field, order_type,'[data-object-index='+object_index+']');
     ...
     */
    $('.page-diy-pc-section .object-lists').on("click",'.btn-object[data-type="column_list"]',function(){
        ...
        if(type=='column_list'){
            column_list_loader(5,0,'create_time','ASC','[data-object-index='+object_index+']');
        }
    });
