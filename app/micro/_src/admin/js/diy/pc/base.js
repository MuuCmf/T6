 ...
 * Copyright (c) 2014-2016 muucmf.cn; Licensed MIT
 ...
 (function($, window, undefined) {
    ...
    let header = function(){
        //点击显示头部控制区
        $('.page-header').on("click",'.config-preview',function(){
            //已经显示的不再触发
            if($(this).find('.config-controller').hasClass('show')){
                ...
                //隐藏通用头部控制区
                $('.public-header').find('.config-controller').removeClass('show');
    ...
    let sortable = function(options){
        // 拖放排序
        // 定义选项对象
        if($('.page-diy-pc-section').length > 0){
            let options_s = {
                stopPropagation: true,
                selector: '.page-diy-pc-section .object-item',
                finish: function(e) {
            ...
            //调用排序
            $('.page-diy-pc-section .preview-target').sortable(options);
            // 解除文本框默认事件禁用
    ...
    let close_header_fooder = function(){
        $('.page-diy-pc-section').on("click",'.object-item',function(){
    ...
    let remove_object = function(ele){
        //点击删除按钮移除组件DOM
        $('.page-diy-pc-section').on("click",'.del-btn',function(){
    ...
    let up_object = function(){
        $('.page-diy-pc-section').on("click",'.up-btn',function(event){
    ...
    let down_object = function(){
        $('.page-diy-pc-section').on("click",'.down-btn',function(event){
            event.stopPropagation();
            //隐藏控制区,重新触发获取索引
            $(this).siblings('.diy-preview-controller').removeClass('show');
