$(function(){
    ...
    var object_type;
	//点击显示轮播控制区
	$('.page-diy-pc-section').on("click",'[data-type="slideshow"]',function(){
	...
	// //图片上传
	// $('.page-diy-pc-section').on("click",'.slideshow .uploadImgBtn',function(e){
	// 	var _this = $(this).parents('.object-item');
	// 	//console.log('[data-object-index="'+object_index+'"]');
	// 	util.image('[name="img_url"]', function(data){
	// 		//console.log(data);
	// 		//获取链接至组件HTL
	// 		var link_html = _this.find('.link-box').html();

	// 		var html_item = '';
	// 			html_item += '<div class="form-horizontal slideshow-form-item" data-rule="object-controller-item">';
	// 			html_item += '<input type="hidden" name="img_id" value="'+data.id+'"/>';
	// 			html_item += '<input type="hidden" name="img_url" value="'+data.attachment+'"/>';
	// 			html_item += '<div class="form-group">';
	// 			html_item += '	<div class="col-xs-12 slideshow-item"><img src="'+data.url+'"/></div>';
	// 			html_item += '</div>';
	...
	//点击删除按钮移除组件DOM
	$('.page-diy-pc-section').on("click",'.del-item-btn',function(){
		//获取索引
	...
	$('.page-diy-pc-section .object-lists').on("click",'.btn-object[data-type="slideshow"]',function(){
