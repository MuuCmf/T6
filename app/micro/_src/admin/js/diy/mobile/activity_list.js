$(function(){
    ...
    var object_type;
    //列表数据接口
    var list_api = $('.btn-object[data-type="activity_list"]').data('list-api');
    //分类数据接口
    var category_api = $('.btn-object[data-type="activity_list"]').data('category-api');
    //列表加载初始数据
    let activity_list_loader = function(rows=2, order_field= 'create_time', order_type= 'DESC', type= 'all', category_id= 0, rank = 1, style = 0, element){
        ...
        $.get(url,function(data){
            ...
            if(data.code){
                if(data.data){
                    $.each(data.data.data,function(i,n){
                        ...
                        if(rank == 1){
                            //小图显示
                            if(style == 0){
                                html_text += '<div class="item small clearfix">';
                                ...
                                html_text += '<div class="start-time">' + n.start_time_sim + ' ' + n.start_time_week +'</div>';
                                ...
                                if(show_sale == 1){
                                    ...
                                    html_text += '   <i class="fa fa-users" aria-hidden="true"></i> ' + n.sales;
                                    html_text += ' </div>';
                                }
                                if(show_favorites == 1){
                            ...
                            //大图显示
                            if(style == 1){
                                html_text += '<div class="item big clearfix">';
                                html_text += '<div class="image"><img src="'+ n.cover_400+'" />';
                                html_text += '<div class="content">';
                        ...
                        //横排DOM
                        if(rank == 0){
                            html_text += '<div class="item clearfix">';
                            html_text += '<div class="image"><img src="'+ n.cover_200+'" /></div>';
                            html_text += '<div class="content">';
                                html_text += '<div class="title h3 text-ellipsis-2">'+ n.title +'</div>';
                ...
                if(rank == 1){
                    $(element).find('.activity-list-preview .list').removeClass('rank0');
                    $(element).find('.activity-list-preview .list').addClass('rank1');
                }else{
                    $(element).find('.activity-list-preview .list').removeClass('rank1');
                    $(element).find('.activity-list-preview .list').addClass('rank0');
                }

                $(element).find('.activity-list-preview .list').html(html_text);
            }
        });
    }

    //点击显示列表控制区
    $('.page-diy-section').on("click",'[data-type="activity_list"]',function(e){
        ...
        $('.page-diy-section').on('click','[data-object-index="'+object_index+'"] .btn',function(){
            ...
            //执行重新加载ajax数据
            activity_list_loader(rows,order_field,order_type,type,category_id,rank,style,'[data-object-index='+object_index+']');
        });
     ...
     */
    $('.page-diy-section .object-lists').on("click",'.btn-object[data-type="activity_list"]',function(){
        ...
        $.get(url, function($res){
            ...
            if($res.code == 200){
                ...
                //获取初始列表数据
                activity_list_loader(2,'create_time','ASC','all',0,1,0,'[data-object-index='+object_index+']');
                
            }else{
                toast.error('该组件需安装线下活动扩展模块','danger');
                return;
            }
        });
    });
