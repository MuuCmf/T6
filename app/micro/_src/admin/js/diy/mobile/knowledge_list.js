...
$(function(){
    ...
    //分类数据接口
    var category_api = $('.btn-object[data-type="knowledge_list"]').data('category-api');
    //课程列表加载初始数据
    let knowledge_list_loader = function(rows= 2, order_field= 'create_time', order_type= 'DESC', type= 'all', category_id= 0, rank = 1, style = 0,element){
        ...
        $.get(url,function(data){
            ...
            if(data.code){
                ...
                if(rank == 1){
                    $(element).find('.knowledge-list-preview .list').removeClass('rank0');
                    $(element).find('.knowledge-list-preview .list').addClass('rank1');
                }else{
                    $(element).find('.knowledge-list-preview .list').removeClass('rank1');
	...
	//点击显示列表控制区
	$('.page-diy-section').on("click",'[data-type="knowledge_list"]',function(e){
		...
		$('.page-diy-section').on('click','[data-object-index="'+object_index+'"] .btn',function(e){
			...
			//console.log(rank);
			//执行重新加载ajax数据
			knowledge_list_loader(rows,order_field,order_type,type,category_id,rank,style,'[data-object-index='+object_index+']');
        });
     ...
     */
    $('.page-diy-section .object-lists').on("click",'.btn-object[data-type="knowledge_list"]',function(){
        ...
        if(type=='knowledge_list'){
            knowledge_list_loader(2,'create_time','ASC','all',0,1,0,'[data-object-index='+object_index+']');
        }
    });
