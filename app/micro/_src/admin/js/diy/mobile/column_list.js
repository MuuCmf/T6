...
$(function(){
    ...
    //专栏列表加载初始数据
    let column_list_loader = function(rows=2, category_id= 0, order_field= 'create_time', order_type= 'DESC', rank = 1,element){
        ...
        $.get(url,function(data){
            ...
            if(data.code){
                if(data.data){
                    ...
                    if(rank == 1){
                        $(element).find('.column-list-preview .list').removeClass('rank0');
                        $(element).find('.column-list-preview .list').addClass('rank1');
                    }else{
                        $(element).find('.column-list-preview .list').removeClass('rank1');
                        $(element).find('.column-list-preview .list').addClass('rank0');
                    }
                    //写入DOM
                    $(element).find('.column-list-preview .list').html(html_text);
                }
            }
        });
    }
	...
	//点击显示专栏列表控制区
	$('.page-diy-section').on("click",'[data-type="column_list"]',function(){
		//已经显示的不再触发
		...
		$('.page-diy-section').on('click','[data-object-index="'+object_index+'"] .btn',function(){
			...
			//执行重新加载ajax数据
			column_list_loader(rows, category_id, order_field, order_type, rank,'[data-object-index='+object_index+']');
		});
