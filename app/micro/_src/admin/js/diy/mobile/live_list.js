...
$(function(){
	...
	var object_type;
    //列表数据接口
    var list_api = $('.btn-object[data-type="live_list"]').data('list-api');
    //分类数据接口
    var category_api = $('.btn-object[data-type="live_list"]').data('category-api');
    //课程列表加载初始数据
    let live_list_loader = function(rows=2, order_field= 'create_time', order_type= 'ASC', category_id= 0, rank = 1, style = 0, element){
                    ...
                    $.each(data.data.data,function(i,n){
                        ...
                        if(rank == 1){
                            ...
                            if(style == 0){
                                ...
                                html_text += '<div class="image"><img src="'+ n.cover_200+'" /><div class="type-container">直播</div></div>';
                            ...
                            if(style == 1){
                                ...
                                html_text += '<div class="image"><img src="'+ n.cover_400+'" /><div class="type-container">直播</div></div>';
                                html_text += '<div class="content">';
	...
	//点击显示直播列表控制区
	$('.page-diy-section').on("click",'[data-type="live_list"]',function(e){
		...
		$('.page-diy-section').on('click','[data-object-index="'+object_index+'"] .btn',function(){
			...
			//console.log(rank);
			//执行重新加载ajax数据
			live_list_loader(rows,order_field,order_type,category_id,rank,style,'[data-object-index='+object_index+']');
		});
     ...
     */
    $('.page-diy-section .object-lists').on("click",'.btn-object[data-type="live_list"]',function(){
        ...
        $.get(url, function($res){
            ...
            if($res.code == 200){
                ...
                //获取初始列表数据
                live_list_loader(2,'create_time','ASC',0,1,0,'[data-object-index='+object_index+']');
            }else{
