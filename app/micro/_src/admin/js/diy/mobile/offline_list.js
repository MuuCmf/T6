...
$(function(){
    ...
    //分类数据接口
    var category_api = $('.btn-object[data-type="offline_list"]').data('category-api');
    //课程列表加载初始数据
    let offline_list_loader = function(rows=2, order_field= 'create_time', order_type= 'ASC', category_id= 0, rank = 1, style = 0, element){
        ...
        //默认加载接口数据
        let url = list_api + '?category_id='+category_id+'&rows='+rows+'&order_field='+order_field+'&order_type='+order_type+'&status=1';
        let html_text = '';
        $.get(url,function(data){
            ...
            if(data.code){
                    ...
                    $.each(data.data.data,function(i,n){
                        ...
                        if(rank == 1){
                            ...
                            if(style == 0){
                                ...
                                html_text += '<div class="start-time">' + n.lessons_start_time_sim + ' ' + n.lessons_start_time_week +'</div>';
                            ...
                            if(style == 1){
                                ...
                                html_text += '<div class="image"><img src="'+ n.cover_400+'" /><div class="type-container">线下课</div></div>';
                                html_text += '<div class="content">';
                ...
                if(rank == 1){
                    $(element).find('.offline-list-preview .list').removeClass('rank0');
                    $(element).find('.offline-list-preview .list').addClass('rank1');
                }else{
                    $(element).find('.offline-list-preview .list').removeClass('rank1');
                    $(element).find('.offline-list-preview .list').addClass('rank0');
                }
	...
	//点击显示列表控制区
	$('.page-diy-section').on("click",'[data-type="offline_list"]',function(e){
		...
		$('.page-diy-section').on('click','[data-object-index="'+object_index+'"] .btn',function(){
			...
			console.log(rank);
			//执行重新加载ajax数据
			offline_list_loader(rows,order_field,order_type,category_id,rank,style,'[data-object-index='+object_index+']');
		});
     ...
     */
    $('.page-diy-section .object-lists').on("click",'.btn-object[data-type="offline_list"]',function(){
        ...
        //获取初始列表数据
        offline_list_loader(2,'create_time','ASC',0,1,0,'[data-object-index='+object_index+']');
