    ...
    private function initFooter($data = '', $style)
    {
        if(!empty($data)) {
            ...
            $data = [
                ...
                'data' => [[
                    'nav_title' => '首页',
                    'icon_url' => request()->domain() . '/static/classroom/images/icon/'. $style .'/gongsi.png',
                    'link' => [
                        "type" => "index",
                        "title" => "首页"
                    ]
                    ...
                    'nav_title' => '我的',
                    'icon_url' => request()->domain() . '/static/classroom/images/icon/'. $style .'/wo.png',
                    'link' => [
                        "type" => "user",
                        "title" => "我的"
                    ]
        ...
        if(!empty($data)) {
            ...
            if(!empty($navtar_arr['head_nav']) && isset($navtar_arr['head_nav'])){
                foreach($navtar_arr['head_nav'] as &$v){
                    if(!empty($v['link'])){
                        $v['link']['url'] = (new \app\classroom\logic\Page())->linkToUrl($v['link'], 'pc');
                    }
