 ...
 * @return bool|string
 */
function send_verify($account, $verify, $type)
{
    switch ($type) {
        case 'mobile':
            //发送手机短信验证
            ...
            $res = send_sms($account, $content, $sendType, $param);
            return $res;
            break;
        case 'email':
            //发送验证邮箱
            $content = config('system.REG_EMAIL_VERIFY');
            $content = str_replace('{$verify}', $verify, $content);
            $content = str_replace('{$account}', $account, $content);
 ...
 * @auth 大蒙
 */
function send_sms($mobile, $content, $type = 'verify', $param = '')
{
    $sms_driver = config('extend.sms_driver');
...
{
    $site_name = config('system.WEB_SITE_NAME');
    if ($to == '') {
