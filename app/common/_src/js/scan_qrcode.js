...
var scan_qrcode = {
    ...
    initWechatQrcode(elem){
        this.createSceneKey();
        let qrCodeSrc = `${this.apiHost}/api/wechat/qrcode?scene_key=${this.sceneKey}`;
    ...
    hasScan(){
        let that = this;
        $.get( `${this.apiHost}/api/wechat/has_scan`,{scene_key:this.sceneKey},function (res) {
            if (res.code == 200){
    ...
    login(){
        ...
        $.post( `${this.apiHost}/api/wechat/scan_login`,{openid:this.userInfo.openid,scene_key:this.sceneKey},function (data) {
            if (data.code == 200) {
