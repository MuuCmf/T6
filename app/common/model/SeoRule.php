...
class SeoRule extends Model
{
    ...
    {
        $result = $this->getMeta(app('http')->getName(), Request()->controller(), Request()->action());
        dump($result);
        //替换META中的变量
    ...
    private function getMeta($app, $controller, $action)
    {
        //查询缓存，如果已有，则直接返回
        ...
        //获取相关的规则
        $rules = $this->getRelatedRules($app, $controller, $action);
     ...
     * 查询当前页SEO规则
     */
    private function getRelatedRules($app, $controller, $action)
    {
        //查询与当前页面相关的SEO规则
        $map['app'] = [['=',''],['=',$app],'or'];
